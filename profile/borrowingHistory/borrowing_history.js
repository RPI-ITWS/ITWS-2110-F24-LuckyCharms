async function borrowingHistory(isAdmin=false, getAll=true, currentPage=1, searchValue="") {
  ...
  let currentlyBorrowingButton
  if (!getAll)
    currentlyBorrowingButton = tabBarButtons[0];
  else
    currentlyBorrowingButton = tabBarButtons[1];

  currentlyBorrowingButton.id = 'chosen';
  const pagination = await fetch(`../../backend/queries/totalReservations.php?getAllReservations=${getAll}&searchValue=${searchValue}`).then((res) => res.json());
  const totalPages = Math.ceil(pagination.totalReservations / 10);
  ...
  function getStatus(reservation) {
    if (reservation.cancelled === 1)
      return "<div class='status cancelled'>Cancelled</div>";
    if (reservation.completed === 1)
      return "<div class='status completed'>Completed</div>";
    let expectedReturnDate;
    if (reservation.date_expected_to_return)
      expectedReturnDate = new Date(reservation.date_expected_to_return);
    if (expectedReturnDate < today && !reservation.date_returned && reservation.borrowable !== 0)
      return "<div class='status danger'>OVERDUE - NEEDS TO BE RETURNED</div>";
    return "<div class='status ongoing'>Ongoing</div>";
  }

  function getDate(reservation) {
    if ((reservation.cancelled === 1 || reservation.completed === 1) && reservation.date_returned)
      return new Date(reservation.date_returned).toLocaleString();
    return "N/A";
  }

  function getExpectedDate(reservation) {
    if (reservation.borrowable === 0)
      return "Not Borrowable - No Return";
    return new Date(reservation.date_expected_to_return).toLocaleString();
  }
...
async function search(event=null) {
  ...
  if (current.innerText === "Borrowing History")
    await borrowingHistory(isAdmin, true, 1, searchValue);
  else
