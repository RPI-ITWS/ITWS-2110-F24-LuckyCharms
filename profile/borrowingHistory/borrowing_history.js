async function borrowingHistory(isAdmin=!1,getAll=!0,currentPage=1,searchValue=""){const current=document.getElementById('chosen');if(current)
  ...
  await borrowingHistory(parseInt(isAdmin),!0,1,searchValue);else await borrowingHistory(isAdmin,!1,1,searchValue);}
  async function completeReservation(reservationId){const date=new Date().toLocaleString();const response=await fetch(`../../backend/queries/completeReservation.php?reservationId=${reservationId}&returnDate=${date}`).then((res)=>res.text()).catch(err=>console.err(err));await borrowingHistory(!0,!1)}
