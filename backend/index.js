...
async function getLocation(htmlElement) {
    const userId = htmlElement.parentElement[0].value;
    if (!userId) {
        console.warn("Please input a user ID");
        return;
    }

    await fetch(`./queries/getLocations.php?userId=${userId}`)
      .then((response) => response.text())
      .then((result) => {
          if (isJsonString(result))
              result = JSON.parse(result);
          console.log(result);
          let options = `<select name="locations" form="filter-search-items">`
...
async function searchItem(htmlElement) {
    const location = htmlElement.parentElement[0].value;
    const searchValue = htmlElement.parentElement[1].value;
    const isBorrowable = htmlElement.parentElement[2].checked;
